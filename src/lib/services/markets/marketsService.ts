...
class MarketsServiceImpl implements MarketsService {
  ...
  async fetchMarkets(
    ...
    const cacheKey = `markets_${markets
      .map((m) => m.symbol)
      .sort()
      .join('_')}${forceRefresh ? `_${Date.now()}` : ''}`;
          ...
          this.limit(async () => {
            try {
              const response = await fetch(
                `https://finnhub.io/api/v1/quote?symbol=${market.symbol}&token=${FINNHUB_API_KEY}`
              );
  ...
  async searchSymbols(query: string): Promise<Market[]> {
    ...
    return cache.getOrFetch(
      ...
      async () => {
        ...
        const promises = topResults.map((result) =>
          this.limit(async () => {
            try {
              const response = await fetch(
                `https://finnhub.io/api/v1/quote?symbol=${result.symbol}&token=${FINNHUB_API_KEY}`
              );
              if (response.status === 429) {
                // Rate limited, wait and retry once
